...
## Overview
399 pathogen-host peptide pairs spanning 32 organisms implicated in autoimmune rheumatic diseases and Guillain-Barré syndrome. Labels assigned based on TM-align structural validation (RMSD threshold 2.0 Å).
...
| BLOSUM80_per_residue | float | BLOSUM80 score divided by alignment length — derived feature (0 for N-class) |
...
- **Non-Mimics**: RMSD ≥ 2.0 Å (n = 137)

## Missing Values
- BLOSUM-derived features and sequence alignment columns are zero-coded for N-class entries (negatives constructed by cross-pairing have no biologically meaningful BLAST signal)

## Key Findings (from analysis)
- Sequence identity explains at most 1.6% of variance in structural RMSD (r = −0.127, p = 0.036, R² = 0.016, n = 272, all Y-class entries with identity data)
- Within the strictly corrected Y-class subset (n = 262), r = −0.082, p = 0.187, R² = 0.007 — both values are near-zero and the relationship is not practically predictive
...
- Mann-Whitney U tests show highly significant Y vs N differences (p < 0.0001) for all sequence features, but these reflect categorical class construction rather than independent sequence discrimination — Layer A (Y vs genuine biological negatives, n=9) and Layer C (strong vs weak mimics) show no significant separation
- Random Forest classifier achieved AUC = 0.958 (Y vs N); AUC dropped to 0.841 on the biologically valid strong-vs-weak task

## Notes
Negative examples (N class) were constructed by cross-pairing pathogen peptides with human peptide matches drawn from the same pre-filtered pool of BLAST-confirmed sequence-similar proteins, paired with a different pathogen peptide than their original match.

See manuscript Methods Section 3.4.3 and Discussion Section 5.1–5.3 for full details and associated limitations.
